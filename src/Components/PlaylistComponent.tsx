...
import { SmallSingleColumnVideo, UserImage } from "./VideoComponent";
...
export const PlaylistPage: React.FC<PlaylistPageProps> = ({
  ...
  return (
    <>
      <Head>
        <title>{playlist?.title ? playlist?.title + " - Midnight" : ""}</title>
    ...
    <div className=" mx-auto grid grid-cols-1 gap-x-4 gap-y-8 md:mx-0 md:max-w-none md:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-2 xl:mx-0 xl:max-w-none xl:grid-cols-2 2xl:mx-0 2xl:max-w-none 2xl:grid-cols-2  ">
      {playlists?.map((playlist) => (
        <Link
          ...
          className="flex flex-col items-start justify-between hover:bg-neutral-700 rounded-xl p-2"
          key={playlist.id}
        >
          <SinglePlaylist playlist={playlist}>
            <p className="text-regular mt-2 max-h-12 overflow-hidden text-neutral-300">
        ...
        <div className="items-top relative mt-4 flex gap-x-4 ">
          <div className=" w-full ">
            <div className=" w-100 flex ">
              <h3 className="h-auto w-full text-2xl font-medium text-neutral-100 group-hover:text-neutral-300">
                {playlist.title}
              </h3>
            </div>
            {children}
