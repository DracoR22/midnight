...
export const MuliColumnVideo: React.FC<VideoComponentProps> = ({videos, users}) => (
  <div className=" mx-auto grid grid-cols-1 gap-x-4 gap-y-8 md:mx-0 md:max-w-none md:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-3 xl:mx-0 xl:max-w-none xl:grid-cols-3 2xl:mx-0 2xl:max-w-none 2xl:grid-cols-3  ">
    {videos.map((video, index) => {
      ...
      return (
        <Link
          href={`/video/${video.id}`}
          className="flex flex-col items-start justify-between hover:bg-neutral-700"
          key={video.id}
        >
...
}: { title: string; limitHeight?: boolean; limitSize?: boolean;}) {
  return (
    <h1
      className={`max-w-md font-semibold leading-6 text-neutral-100 group-hover:text-neutral-300 ${
...
export function VideoInfo({
  ...
  return (
    <div className="mt-1 flex max-h-6 items-start overflow-hidden text-sm">
      ...
      {/* SEE WHEN THE VIDEO WAS CREATED FROM NOW */}
      <p className=" text-neutral-300">{moment(createdAt).fromNow()}</p>
    </div>
  );
}
...
export function UserImage({
  ...
  export function UserName({ name }: { name: string }) {
    return (
      <p className="max-h-6 overflow-hidden text-sm font-semibold leading-6 text-neutral-100">
        {name}
      </p>
    );
  }